...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.meas_time = ctk.StringVar(value="30")
        ...
        self.measure_intern_co2 = ctk.BooleanVar(value=False)
        ...
        now = datetime.now()
        filename = now.strftime("data_%Y_%m_%d_%H%M.csv")
    ...
    def init_main_frame(self, parent):
        ...
        self.temp_box = ctk.CTkCheckBox(frame, text="Temperature", variable=self.measure_temp, command=self.wind_selected)
        self.temp_box.grid(row=5, column=0, padx=5, sticky="w")
        self.pres_box = ctk.CTkCheckBox(frame, text="Pressure", variable=self.measure_press, command=self.wind_selected)
        self.pres_box.grid(row=5, column=1, padx=5, sticky="w")
        self.wind_box = ctk.CTkCheckBox(frame, text="Wind", variable=self.measure_wind, command=self.wind_selected)
        self.wind_box.grid(row=6, column=0, padx=5, pady=5, sticky="w")
        self.h2o_box = ctk.CTkCheckBox(frame, text="H2O", variable=self.measure_hum, command=self.co2_selected)
        self.h2o_box.grid(row=6, column=1, padx=5, pady=5, sticky="w")
        self.co2_box = ctk.CTkCheckBox(frame, text="CO2", variable=self.measure_co2, command=self.co2_selected)
        self.co2_box.grid(row=7, column=0, padx=5, sticky="w")
        self.intern_co2_box = ctk.CTkCheckBox(frame, text="P and T (CO2)", variable=self.measure_intern_co2, command=self.co2_selected)
        self.intern_co2_box.grid(row=7, column=1, padx=5, sticky="w")

        # Amount of up & down and repetitions
        ctk.CTkLabel(frame, text="Measurement time (min):").grid(row=8, column=0, padx=5, pady=(10, 0), sticky="w")
        ctk.CTkEntry(frame, textvariable=self.meas_time).grid(row=8, column=1, padx=5, pady=(10, 0), sticky="w")

        # Measuring mode (continuous / discontinuous)
        ctk.CTkLabel(frame, text="Measuring mode :").grid(row=9, column=0, padx=5, pady=(10, 0), sticky="w")
        self.mode_menu = ctk.CTkOptionMenu(frame, values=["Continuous", "Discontinuous"], variable=self.mode, command=self.toggle_mode)
        self.mode_menu.grid(row=9, column=1, padx=5, pady=(10, 0), sticky="w")
        
        # Acquisition settings
        self.freq_label = ctk.CTkLabel(frame, text="Frequency (Hz) (< 20) :")
        ...
        self.freq_label.grid(row=10, column=0, padx=5, pady=(10, 0), sticky="w")
        self.freq_entry.grid(row=10, column=1, pady=(10, 0), sticky="w")
        self.nb_label.grid(row=10, column=0, padx=5, pady=(10, 0), sticky="w")
        self.nb_entry.grid(row=10, column=1, pady=(10, 0), sticky="w")
        self.nb_label.grid_remove()
        self.nb_entry.grid_remove()

        
        ctk.CTkLabel(frame, text="").grid(row=11, column=0, columnspan=2, pady=20)
        ...
        self.progress_label.grid(row=12, column=2, padx=5, pady=(0, 10), sticky="ew")
        self.progress_label.grid_remove()

        # Start & stop buttons settings and availability
        self.find_button = ctk.CTkButton(frame, text="Find Ground", fg_color="#E6B800",  hover_color="#806808", command=self.finding_ground)
        self.find_button.grid(row=13, column=0, columnspan=2, padx=5, pady=10, sticky="w")
        self.find_button.configure(state="disabled")

        self.start_button = ctk.CTkButton(frame, text="Start", fg_color="green", hover_color="#0B4D23", command=self.start_recording)
        self.start_button.grid(row=13, column=1, columnspan=2, padx=5, pady=10, sticky="w")
        self.start_button.grid_remove()

        self.stop_button = ctk.CTkButton(frame, text="Stop", fg_color="red", hover_color="#9B0606", command=self.stop_recording)
        self.stop_button.grid(row=13, column=0, pady=10, sticky="w")
        self.stop_button.grid_remove()

        self.valid_button = ctk.CTkButton(frame, text="Update Value", fg_color="green", hover_color="#0B4D23", command=self.update)
        self.valid_button.grid(row=13, column=1, pady=10, sticky="w")
    ...
    def wind_selected(self):
        if self.measure_wind.get() or self.measure_temp.get() or self.measure_press.get():
    ...
    def co2_selected(self):
        if self.measure_co2.get() or self.measure_hum.get() or self.measure_intern_co2.get():
    ...
    def connect(self):
        self.connected_button.configure(state="disabled")
        ...
        intern_co2 = self.measure_intern_co2.get()
        self.result = [com, ip, port, vendor, product, temp, pres, wind, hum, co2, intern_co2]
    ...
    def unconnect(self):
        ...
        self.connected_button.configure(state="normal")
    ...
    def find_ground(self, i):
        if i == 0:
            self.gposition_button.configure(state="normal")
            self.gsave_button.configure(state="normal")
    ...
    def save_ground_field(self):
        self.gposition_button.configure(state="disabled")
        self.gsave_button.configure(state="disabled")
        if self.is_number(self.gposition.get(), "distance to the ground in field", -500, 0):
            gposition = int(self.gposition.get())
        else:
            return
    ...
    def start_recording(self):
        ...
        if self.mode.get() == "Continuous":
            if self.is_number(self.freq.get(), "frequency", 0, 20):
            ...
            if self.is_number(self.nb_points.get(), "measurement points", 0):
                ...
                return
            freq = 1
        if self.is_number(self.meas_time.get(), "measurement time", 0):
            pass
        else:
            return
        temp = self.measure_temp.get()
        ...
        co2 = self.measure_co2.get()
        intern_co2 = self.measure_intern_co2.get()
        self.result = [filename, temp, pres, wind, hum, co2, intern_co2, mode, nb_points, freq]
        ...
        self.intern_co2_box.configure(state="disabled")
    ...
    def stop_recording(self):
        ...
        if messagebox.askyesno("Stop", "Do you want to stop measuring ?"):
            ...
            if self.app_state == "record":
                self.valid_button.configure(state="disabled")
    ...
    def update(self):
        if self.is_number(self.meas_time.get(), "measurement time", 0):
            meas_time = int(self.meas_time.get())
        else:
            return
        self.result = [meas_time]
    ...
    def get_actual_state(self):
        ...
        if self.app_state in ["find_ground","saveground"]:
            if self.finding_ground_label.winfo_ismapped():
                self.finding_ground_label.grid_remove()
            else:
        ...
        if self.app_state == "ground_fond":
            ...
            self.gposition_input.configure(state="disabled")
            self.gposition_button.configure(state="disabled")
            self.gsave_button.configure(state="disabled")
            self.app_state = "wait"
